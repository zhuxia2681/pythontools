...
# JDY 20OCT2020 
# TODO take care for band numbers.
#
...
wfng_file = "./4-sapo/wfn.sapo.cplx.hdf5"
wfng_nband = 2538
rhog_flag = False
rhog_file = "RHO"
eig_flag = True
...
# from BGW get parameter for header
with h5py.File(wfng_file, 'r') as h:
  ...
  ngk = tmp[...]
  tmp = h['mf_header/kpoints/rk']
  rk = tmp[...]
  tmp = h['mf_header/kpoints/kgrid']
  kgrid = tmp[...]
...
def write_evc(input_file_name,real_or_complex,wfng_nband,output_dir_name ):
  ...
  tree = get_xml_data(output_dir_name)
  root = tree.getroot()
...
# @@ JDY I must combine up and dw to one unified array
# 20OCT2020

  else:
    igk = 0
    for i in range(nk):
      wfcupname = output_dir_name + "wfcup"+str(i+1)+".hdf5"
      wfcdwname = output_dir_name + "wfcdw"+str(i+1)+".hdf5"
      tmpup = np.zeros((mnband,ngk[i]*2))
      tmpdw = np.zeros((mnband,ngk[i]*2))
      for k in range(mnband):
        n = 0
        for j in range(ngk[i]):
          for l in range(2):
            tmpup[k,n] = coeffs[k,0,igk+j,l]
            tmpdw[k,n] = coeffs[k,1,igk+j,l]
            n += 1
      with h5py.File(wfcupname,"w") as hh:
        hh.create_dataset('MillerIndices',data=gvecs[igk:ngk[i],:])
        hh.create_dataset('evc',data=tmpup[:,:])
        hh.attrs['ik'] = i
        hh.attrs['xk'] = rk[i]
        hh.attrs['ispin'] = 0
        hh.attrs['gamma_only'] = gamma_only
        hh.attrs['scale_factor'] = 1.0
        hh.attrs['ngw'] = ngk[i]
        hh.attrs['igwx'] = ngk[i]
        hh.attrs['npol'] = 1
        hh.attrs['nbnd'] = mnband
#        hh.create_dataset('evc',data=coeffs[:,0,igk:ngk[i],:])
      with h5py.File(wfcdwname,"w") as hh:
        hh.create_dataset('MillerIndices',data=gvecs[igk:ngk[i],:])
        hh.create_dataset('evc',data=tmpdw[:,:])
        hh.attrs['ik'] = i
        hh.attrs['xk'] = rk[i]
        hh.attrs['ispin'] = 1
        hh.attrs['gamma_only'] = gamma_only
        ...
        hh.attrs['npol'] = 1
        hh.attrs['nbnd'] = mnband
#        hh.create_dataset('evc',data=coeffs[:,1,igk:ngk[i],:])
      igk = igk + ngk[i]
...
# get a lattice
# TODO summarize and put on hea part
#
  ...
  xml_a[1,:] = xml_a2 
  xml_a[2,:] = xml_a3
  inv_xml_a = np.linalg.inv(xml_a)


# check k points. we must check array rk.

  nbns = nspin * mnband
  n = 0
  chk_k = np.zeros((3))
  xml_chk_k = np.zeros((3))
  for i in root.findall('./output/band_structure/ks_energies'):
    tmp = i[0].text
    tmp = tmp.split()
    chk_k = np.array(tmp).astype(float)
    xml_chk_k = np.dot(inv_xml_a,rk[n])
    if chk_k.all() != xml_chk_k.all():
      print("Error k point not match data.xml <=> BGW.wfn")
      sys.exit()
    xml_chk_npw = int(i[1].text)
    if ngk[n] != xml_chk_npw:
      print("Error number of pw not match data.xml <=> BGW.wfn")
      sys.exit()
    i[2].attrib['size'] = str(nbns)
    if len(eig[:,n]) != nbns:
      print("Error number of eigenvalues not match data.xml <=> BGW.wfn")
      sys.exit()
    tmp = eig[:,n].tolist()
    tmp = str(tmp)
    tmp = tmp[1:-2]
    ...
    i[3].attrib['size'] = str(nbns)
#    tmp = np.stack([occ[0,n,:],occ[1,n,:]],0)
#    print(tmp)
    tmp1 = occ[0,n,:].tolist()
    tmp2 = occ[1,n,:].tolist()
    tmp1[len(tmp1):len(tmp1)]=tmp2
#    tmp3 = tmp3.tolist()
#    tmp1 = str(tmp1)
#    tmp1 = re.sub("\[\]","",tmp1)
    tmp1 = str(tmp1)
